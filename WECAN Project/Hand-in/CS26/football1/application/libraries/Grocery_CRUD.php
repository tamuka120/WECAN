...
class grocery_CRUD_Layout extends grocery_CRUD_Model_Driver
{
	...
	protected function delete_layout($delete_result = true)
	{
		...
		if($delete_result === false)
		{
			$error_message = '<p style="color:white">'.$this->l('delete_error_message').'</p>';

			echo json_encode(array('success' => $delete_result ,'error_message' => $error_message));
		}
		else
		{
			$success_message = '<p style="color:white">'.$this->l('delete_success_message').'</p>';
	...
	protected function insert_layout($insert_result = false)
	{
		...
		{
			$success_message = '<p style="color:white">'.$this->l('insert_success_message');
	...
	protected function update_layout($update_result = false, $state_info = null)
	{
		...
		{
			$success_message = '<p style="color:white">'.$this->l('update_success_message');
			if(!$this->unset_back_to_list && !$this->_is_ajax())
			{
				$success_message .= " <a href='".$this->getListUrl()."'>".$this->l('form_go_back_to_list')."</a>";
			}
	...
	protected function get_string_input($field_info,$value)
	{
		...
		if(!empty($field_info->db_max_length))
			$extra_attributes .= "maxlength='{$field_info->db_max_length}'";
		$input = "<input id='field-{$field_info->name}' name='{$field_info->name}' type='text' value=\"$value\" $extra_attributes required/>";
		return $input;
	}
	...
	protected function get_dropdown_input($field_info,$value)
	{
		...
		$input = "<select id='field-{$field_info->name}' name='{$field_info->name}' class='chosen-select' data-placeholder='".$select_title."' required>";
	...
	protected function get_multiselect_input($field_info,$value)
	{
		...
		$select_title = str_replace('{field_display_as}',$field_info->display_as,$this->l('set_relation_title'));
		$input = "<select id='field-{$field_info->name}' name='{$field_info->name}[]' multiple='multiple' size='8' class='chosen-multiple-select' data-placeholder='$select_title' style='width:510px;' required>";
	...
	protected function get_relation_input($field_info,$value)
	{
		...
		$select_title = str_replace('{field_display_as}',$field_info->display_as,$this->l('set_relation_title'));
		$input = "<select id='field-{$field_info->name}'  name='{$field_info->name}' class='$ajax_or_not_class' data-placeholder='$select_title' style='width:300px' required>";
